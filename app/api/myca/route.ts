...
function generateFallbackResponse(question: string) {
  ...
  if (lowerQuestion.includes("species")) {
    answer +=
      "I can see several active species in the network, including Amanita muscaria and Pleurotus ostreatus. Would you like more details about a specific species?"
  } else if (lowerQuestion.includes("network") || lowerQuestion.includes("connection")) {
    answer +=
      "The mycorrhizal network is showing healthy interconnectivity with multiple active nodes. The clustering coefficient is approximately 0.73."
  } else if (lowerQuestion.includes("compound") || lowerQuestion.includes("chemical")) {
    answer +=
      "Current trending bioactive compounds include Psilocybin, Cordycepin, and Beta-glucan. I'm detecting increased synthesis activity."
  } else if (lowerQuestion.includes("health") || lowerQuestion.includes("status")) {
    answer +=
      "System health is optimal with 42 active devices and normal environmental parameters. All sensors are reporting within expected ranges."
  } else {
...
async function getSuggestedQuestions() {
  try {
    ...
    if (liveData.environmentalReadings?.length > 0) {
      const latestTemp = liveData.environmentalReadings.find((r: any) => r.parameter === "Temperature")
      if (latestTemp) {
