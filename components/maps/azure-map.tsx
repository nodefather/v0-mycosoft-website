...
// ---- Azure Maps dynamic loader --------------------------------------------
function loadAzureMaps(): Promise<any> {
  /* Safely load atlas.min.js & CSS only once */
  if (typeof window === "undefined") return Promise.reject()
  if ((window as any).atlas) return Promise.resolve((window as any).atlas)
  ...
  return new Promise((resolve, reject) => {
    ...
    script.async = true
    script.onload = () => resolve((window as any).atlas)
    script.onerror = reject
    document.head.appendChild(script)

    /* CSS */
    if (!document.querySelector("#atlas-map-css")) {
      const link = document.createElement("link")
      link.id = "atlas-map-css"
      link.rel = "stylesheet"
...
export function AzureMap({ className, deviceLocations = defaultDevices, onDeviceClick }: AzureMapProps) {
  ...
  useEffect(() => {
    ...
    async function initMap() {
      try {
        const atlas = await loadAzureMaps() // <-- NEW

        /* (previous try/catch logic remains identical, but replace every
           reference to the imported `atlas` namespace with this local const) */
        if (!atlas || !authInfo.token || !authInfo.clientId) {
        ...
        const newMap = new atlas.Map(mapRef.current, {
          authOptions: {
            ...
            getToken: async () => {
              const res = await fetch("/api/maps/auth")
              if (!res.ok) throw new Error("Failed to refresh map token")
