...
export default function MycaChatComponent() {
  const [messages, setMessages] = useState([
    {
      ...
      type: "assistant",
      content:
        "Hello! I'm MYCA, your fungal intelligence assistant. I'm connected to the NatureOS MINDEX database and can help you explore the mycological network. What would you like to know?",
      timestamp: new Date(),
    },
  ])
  ...
  const sendFeedback = async (messageId: number, feedbackType: "helpful" | "not-helpful") => {
    try {
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-gray-500">{isConnected ? "Connected to NatureOS" : "Offline"}</span>
            </div>
          </div>
        </div>
      </div>
...
\
                ...
                {message.type === "assistant" && !message.error && (\
                  <div className="mt-2 space-y-2">\
                    ...
                    {message.sources && <div className="text-xs text-gray-500">Sources: {message.sources.join(", ")}</div>}
